...
router.post(
    ...
    body("confirmPassword").isLength({min: 8}).withMessage("confirm password minimum 8 characters")
        .custom((value, {req}) => {
            if(value !== req.body.password) throw new Error("confirmPassword not match")
            return true
        }),
    body("displayName").isLength({min: 8}).withMessage("password minimum 8 characters"),
